...
async function getInitialPrice() {
  ...
  const wsb = "wsb"
  const cootie_ADDRESS = "0xe4671844fcb3ca9a80a1224b6f9a0a6c2ba2a7d5"
  const cootie = "cootie"
  ...
  let cootieSGBPrice = (Number(arryPrice[1]._hex) / 10 ** 18).toFixed(5)
  let wsbUSDPrice = (sgbPrice / wsbSGBPrice).toFixed(5)
  let cootieUSDPrice = (sgbPrice / cootieSGBPrice).toFixed(5)
  console.log(wsb + " USD price is " + wsbUSDPrice)
  console.log(cootie + " USD price is " + cootieUSDPrice)

  clearRoles()
  lastPrice = cootieUSDPrice || 0
  let symbol = `${process.env.COIN_ID.toUpperCase()}`
  client.user.setPresence({
    activities: [{
    name: `SGB=${cootieSGBPrice} ${symbol}`,
    type: `PLAYING`
    }]
  })

  arrow = mid
  client.guilds.cache.find(guild => guild.id === process.env.SERVER_ID).me.setNickname(`${symbol} ${arrow} ${process.env.CURRENCY_SYMBOL}${cootieUSDPrice}`)
